...
export const Header: React.FC = () => {
  ...
  const { employeeUser, isEmployeeAuth, logout: employeeLogout } = useEmployeeAuth();
  ...
  const getSubscriptionIndicator = () => {
    // Se for funcionário, não mostrar indicador de assinatura
    if (isEmployeeAuth) {
      return (
        <div className="flex items-center space-x-2 px-3 py-1.5 rounded-lg text-sm font-medium bg-blue-100 text-blue-800 border border-blue-200">
          <User className="w-4 h-4" />
          <span className="hidden sm:inline">Acesso Funcionário</span>
    ...
    if (hasActiveSub) {
      ...
      return (
        <Link 
          to="/pricing"
          className="flex items-center space-x-2 px-3 py-1.5 rounded-lg text-sm font-medium bg-red-100 text-red-800 border border-red-200"
        >
    ...
    <header className="bg-white border-b border-gray-200 px-4 sm:px-6 py-4 sm:py-5 h-16 sm:h-20 fixed top-0 left-0 right-0 z-40">
      <div className="flex items-center justify-between h-full">
          ...
          {(isEmployeeAuth && employeeUser) && (
            <div className="flex items-center space-x-4 pl-4 sm:pl-6 border-l border-gray-200">
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-orange-100 rounded-xl flex items-center justify-center overflow-hidden">
                {employeeUser?.restaurant_logo ? (
                  <img 
                    src={employeeUser.restaurant_logo} 
                    alt="Logo" 
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
          ...
          {(user || isEmployeeAuth) && getSubscriptionIndicator()}
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="text-sm">
                <div className="font-medium text-gray-700">
                  {isEmployeeAuth ? employeeUser?.name : user?.email}
                </div>
                <div className="text-xs text-gray-500">
                  {isEmployeeAuth ? 'Funcionário' : (user?.tipo_usuario === 'admin' ? 'Administrador' : 'Restaurante')}
                </div>
              </div>
            </div>
